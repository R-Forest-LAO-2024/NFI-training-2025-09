...
To simplify the calculations, it was considered that:
...
NOTE: The phase 2 focused on foresr and RV land cover, but some plots from phase phase 1 and planned for phase 2 were non-forest in the Phase 1 interpretation and still visited.
...
res3_agb_tot |>
  ...
  geom_col(aes(fill = lc_no), col = "darkgrey") +
  theme(legend.position = "none") +
...
kbl(
  tab_agb,
  col.names = c("Land cover", "prop.", "U (perc)", "tot.", "prop.", "U (perc)", "ton (DM)/ha", "U (perc.)", "tot."),
  format.args = list(big.mark = ",", digits = 2),
  booktabs = F
  |>
  add_header_above(c(" " = 1, "Area" = 3, "AGB" = 5)) |>
...
agb_compa |>
  ...
  geom_col(aes(fill = method), col = "grey80", position = position_dodge()) +
  geom_errorbar(aes(group = method, ymin = agb - agb*agb_U/100, ymax = agb + agb*agb_U/100), position = position_dodge()) +
ylim(0,600)
```

The ratio estimator had big impact on EG and RV. Also big influence on uncertainty of Coniferous (CF)
